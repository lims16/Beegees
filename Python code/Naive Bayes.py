...
import numpy as np
import pandas as pd
from likelihood_validation import secure, likelihood, evidence, features
...
for r in range(df.shape[0]):
    ...
    if(posterior_yes[r, 0] == 0 and posterior_no[r, 0] == 0):
        print(r)
        for c in range(len(features)):
            if(likelihood[r, c] == 0 or likelihood[r, len(features) + c] == 0):
                likelihood[r, c] = 1
                likelihood[r, len(features) + c] = 1
        posterior_yes[r, 0] = secure * \
            np.prod(likelihood[r, :len(df.columns) - 1]) / \
            np.prod(evidence[r, :])
        posterior_no[r, 0] = (1 - secure) * \
            np.prod(likelihood[r, len(df.columns) - 1:]) / \
            np.prod(evidence[r, :])
